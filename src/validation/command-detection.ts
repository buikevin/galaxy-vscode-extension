...
/**
 * Picks the most compatible local Composer binary path for the current platform.
 *
 * @param workspacePath Absolute workspace root.
 * @param binaryName Composer binary name without path or extension.
 * @returns Relative command path that best matches the active platform.
 */
function resolveComposerBinaryCommand(
  workspacePath: string,
  binaryName: string,
): string {
  const scriptPath = `vendor/bin/${binaryName}`;
  const windowsScriptPath = `${scriptPath}.bat`;
  if (process.platform !== "win32") {
    return scriptPath;
  }
  if (hasFile(workspacePath, windowsScriptPath)) {
    return windowsScriptPath;
  }
  if (hasFile(workspacePath, scriptPath)) {
    return scriptPath;
  }
  return windowsScriptPath;
}
...
export function detectProjectCommands(
  ...
  const hasShellFiles = hasTrackedExtension(sessionFiles, [
    ".sh",
    ".bash",
    ".zsh",
  ]);
  ...
  const profiles = new Set(
    detectValidationProfiles(workspacePath, sessionFiles),
  );
  ...
  if (
    ...
    !selectedNodeScripts.some(
      (candidate) => candidate.category === "static-check",
    )
  ) {
    addProjectCommand(commands, seenCommands, {
  ...
  if (
    ...
    if (
      ...
      hasFile(workspacePath, "mypy.ini")
    ) {
      addProjectCommand(commands, seenCommands, {
        id: "mypy-check",
        label: "mypy .",
  ...
  if (
    profiles.has("java") &&
    (hasFile(workspacePath, "build.gradle") ||
      hasFile(workspacePath, "build.gradle.kts"))
  ) {
    const gradleExecutable =
      process.platform === "win32" && hasFile(workspacePath, "gradlew.bat")
        ? "gradlew.bat"
        : hasFile(workspacePath, "gradlew")
          ? "./gradlew"
          : "gradle";
    addProjectCommand(commands, seenCommands, {
      id: "gradle-classes",
  ...
  if (makefileText) {
    for (const target of ["lint"]) {
      ...
      addProjectCommand(commands, seenCommands, {
        ...
        cwd: workspacePath,
        kind: "project",
        profile: profiles.has("typescript")
          ? "typescript"
          : profiles.has("javascript")
            ? "javascript"
            : profiles.has("python")
              ? "python"
              : "javascript",
  ...
  if (profiles.has("php") && hasPhpFiles && composerText) {
    if (composerText.includes("phpstan")) {
      const phpstanCommand = `${resolveComposerBinaryCommand(workspacePath, "phpstan")} analyse`;
      addProjectCommand(commands, seenCommands, {
        id: "phpstan-analyse",
    ...
    if (composerText.includes("phpunit")) {
      const phpunitCommand = resolveComposerBinaryCommand(
        workspacePath,
        "phpunit",
      );
      addProjectCommand(commands, seenCommands, {
        id: "phpunit",
        label: phpunitCommand,
  ...
  if (
    ...
    profiles.has("shell") &&
    hasShellFiles &&
    hasFile(workspacePath, ".shellcheckrc")
  ) {
    addProjectCommand(commands, seenCommands, {
      id: "shellcheck-recursive",
      label: "shellcheck recursive",
      command:
        "find . -type f \\( -name '*.sh' -o -name '*.bash' -o -name '*.zsh' \\) -print0 | xargs -0 shellcheck",
      cwd: workspacePath,
      kind: "project",
      profile: "shell",
      category: "lint",
  ...
  if (
    profiles.has("ruby") &&
    hasRubyFiles &&
    hasFile(workspacePath, "Rakefile")
  ) {
    addProjectCommand(commands, seenCommands, {
      id: "rake-test",
      label: "rake test",
