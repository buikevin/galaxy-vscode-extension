...
export function detectProjectCommands(workspacePath: string): Readonly<{
  ...
  const packageScriptMappings: ReadonlyArray<Readonly<{
    script: string;
  ...
  if (hasFile(resolvedWorkspace, 'pyproject.toml') || hasFile(resolvedWorkspace, 'requirements.txt') || hasFile(resolvedWorkspace, 'setup.py')) {
    detectedStack.add('python');
    if (pyprojectText.includes('[tool.ruff]') || hasFile(resolvedWorkspace, 'ruff.toml') || hasFile(resolvedWorkspace, '.ruff.toml')) {
      addCommand(commands, seen, {
        id: 'ruff-check',
        label: 'ruff check .',
        command: 'ruff check .',
    ...
    if (pyprojectText.includes('[tool.mypy]') || hasFile(resolvedWorkspace, 'mypy.ini')) {
      addCommand(commands, seen, {
        id: 'mypy-check',
        label: 'mypy .',
        command: 'mypy .',
  ...
  if (hasFile(resolvedWorkspace, 'build.gradle') || hasFile(resolvedWorkspace, 'build.gradle.kts')) {
    detectedStack.add('gradle');
    const gradleCommand = hasFile(resolvedWorkspace, 'gradlew') ? './gradlew classes' : 'gradle classes';
    addCommand(commands, seen, {
      id: 'gradle-classes',
      label: gradleCommand,
  ...
  if (fs.readdirSync(resolvedWorkspace).some((entry) => entry.endsWith('.csproj') || entry.endsWith('.sln'))) {
  ...
  if (hasFile(resolvedWorkspace, 'composer.json')) {
    ...
    const composerText = readTextFile(path.join(resolvedWorkspace, 'composer.json'));
