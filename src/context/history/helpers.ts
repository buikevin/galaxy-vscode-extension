...
import { ACTIVE_TASK_MEMORY_SOFT_LIMIT, PROJECT_MEMORY_SOFT_LIMIT } from '../entities/constants';
import {
 ...
 */
export function mergeUniqueItems(existing: readonly string[], incoming: readonly string[], maxItems: number): readonly string[] {
...
export function createToolDigest(message: ChatMessage): ToolDigest {
  ...
  return Object.freeze({
    name: toolName,
    success,
    summary: summaryByTool[toolName] ?? summarizeText(message.content, 140) ?? `${toolName} ${success ? 'ok' : 'failed'}`,
    filesRead,
    filesWritten,
    filesReverted,
  });
 ...
 */
export function collectFilesTouched(toolDigests: readonly ToolDigest[]): readonly string[] {
 ...
 */
export function extractAttachments(userMessage: ChatMessage, contextNote?: string): readonly string[] {
  const fromMessage = [
 ...
 */
export function buildWorkingSessionHandoff(turn: WorkingTurn, assistantText: string): string {
 ...
 */
export function inferTaskMemoryTurnKind(turn: WorkingTurn, assistantText: string): 'analysis' | 'implementation' | 'review' | 'validation' | 'repair' {
 ...
 */
export function compactActiveTaskMemory(memory: ActiveTaskMemory): ActiveTaskMemory {
...
export function createSessionSummaryLine(digest: TurnDigest): string {
  return [
    ...
    digest.filesTouched.length > 0 ? `files=${digest.filesTouched.slice(0, 5).join(', ')}` : '',
  ]
    .filter(Boolean)
    .join(' | ');
}
