 ...
 */
async function loadRegistryByFile(
  fileName: string,
): Promise<GalaxyDesignRegistry> {
  const localPath = localRegistryPath(fileName);
  if (fs.existsSync(localPath)) {
    return JSON.parse(
      fs.readFileSync(localPath, "utf-8"),
    ) as GalaxyDesignRegistry;
  }
 ...
 */
async function loadFrameworkRegistry(framework: GalaxyDesignFramework): Promise<
  Readonly<{
...
function describeRegistryFramework(
  framework: GalaxyDesignFramework,
  registryFramework: GalaxyDesignCanonicalFramework,
): string {
  if (framework === registryFramework) {
    return registryFramework;
  }
  return `${registryFramework} (source package used for ${framework})`;
}
...
function summarizeComponent(
  ...
  if (
    ...
    lines.push(
      `Registry dependencies: ${component.registryDependencies.join(", ")}`,
    );
  }
  if (component.props && component.props.length > 0) {
    ...
    if (propNames.length > 0) {
      lines.push(
        `Props: ${propNames.join(", ")}${component.props.length > propNames.length ? " ..." : ""}`,
      );
    }
  }
  return lines.join("\n");
...
function getGroupMembers(
  ...
  const match = Object.entries(registry.groups).find(
    ([key, value]) =>
      key.toLowerCase() === lowered ||
      String(value.name ?? "").toLowerCase() === lowered,
  );
...
export async function galaxyDesignRegistryTool(
  ...
  try {
    const projectInfo = options?.path
      ? getGalaxyDesignProjectInfo(workspaceRoot, options.path)
      : null;
      ...
      if (component) {
        ...
        return Object.freeze({
          ...
          content: [
            ...
            `Registry framework: ${describeRegistryFramework(framework, loaded.registryFramework)}`,
            summarizeComponent(component, exact, groups),
          ].join("\n"),
          meta: Object.freeze({
            framework,
      ...
      if (query) {
        ...
        return Object.freeze({
          ...
          meta: Object.freeze({
            ...
            sampleComponents: Object.freeze(
              matches.map((item) => item.name).slice(0, 20),
            ),
          }),
        });
      }
    ...
    if (query) {
      const canonicalFrameworks: readonly GalaxyDesignCanonicalFramework[] =
        Object.freeze(["react", "vue", "angular", "react-native", "flutter"]);
      const searchResults = (
        await Promise.all(
          canonicalFrameworks.map(async (candidateFramework) => {
            const loaded = await loadFrameworkRegistry(candidateFramework);
            return searchRegistry(loaded.registry, query)
              .slice(0, 5)
              .map((match) => ({ framework: candidateFramework, ...match }));
          }),
        )
      )
        .flat()
      ...
      return Object.freeze({
        ...
        content: [
          ...
          ...searchResults.map(
            (item, index) =>
              `${index + 1}. [${item.framework}] ${item.name} - ${item.type}${item.description ? ` - ${item.description}` : ""}`,
          ),
        ].join("\n"),
        meta: Object.freeze({
          query,
          ...
          sampleComponents: Object.freeze(
            searchResults.map((item) => `${item.framework}:${item.name}`),
          ),
        }),
      });
    }
    return Object.freeze({
      ...
      content: [
        ...
        "Framework aliases: nextjs -> react source registry, nuxtjs -> vue source registry",
        `Summary registry: ${GALAXY_DESIGN_UNPKG_BASE}/registry.json`,
      ...
      meta: Object.freeze({
        ...
        summaryRegistryUrl: `${GALAXY_DESIGN_UNPKG_BASE}/registry.json`,
        resultCount: Object.keys(summaryRegistry.components).length,
        sampleComponents: Object.freeze(
          Object.keys(summaryRegistry.components).slice(0, 24),
        ),
      }),
    });
  } catch (error) {
