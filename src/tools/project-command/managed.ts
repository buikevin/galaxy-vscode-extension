...
import {
  ...
  resolveProjectCommandExecution,
} from "./core";
 ...
 */
function resolveManagedCommandRecord(
  commandId: string,
): ManagedCommandRecord | null {
  const exact = managedCommands.get(commandId);
  if (exact) {
    return exact;
  }

  const byToolCallId = [...managedCommands.values()].filter(
    (record) => record.toolCallId === commandId,
  );
  if (byToolCallId.length === 1) {
    return byToolCallId[0] ?? null;
  }

  const running = [...managedCommands.values()].filter(
...
export function startManagedCommandTool(
  ...
  const resolved = resolveProjectCommandExecution(
    workspacePath,
    commandOrId,
    options?.cwd,
  );
  ...
  const child = resolved.directCommand
    ? (() => {
        const useCommandShell = shouldUseWindowsCommandShell(
          resolved.directCommand.resolvedBinary,
        );
        return spawn(
          resolved.directCommand.resolvedBinary,
          [...resolved.directCommand.args],
          {
    ...
    : (() => {
        ...
        return spawn(
          shell.executable,
          [...shell.commandArgs(resolved.commandText)],
          {
            cwd: resolved.commandCwd,
            stdio: ["ignore", "pipe", "pipe"],
            env: buildShellEnvironment(),
  ...
  const appendChunk = async (chunk: string): Promise<void> => {
    ...
    record.output = `${record.output}${chunk}`.slice(
  ...
  child.on("close", (code) => {
    ...
    void finalize(
      buildManagedMeta(
        { ...record, durationMs, exitCode: Number(code ?? 0) },
        tailOutput,
        truncated,
      ),
    );
...
export async function awaitManagedProjectCommandTool(
  ...
  if (!meta) {
    const { tailOutput, truncated } = buildTailOutput(
      record.output,
      Math.max(options?.maxChars ?? 8_000, 500),
    );
    return Object.freeze({
      success: true,
  ...
  return Object.freeze({
    ...
    ...(meta.exitCode === 0
      ? {}
      : { error: `Project command failed: ${meta.commandLabel}` }),
    meta,
  });
}
