...
import {
  ASYNC_COMMAND_HANDOFF_MS,
  BACKGROUND_STARTUP_GRACE_MS,
  MAX_CAPTURED_OUTPUT_CHARS,
} from "./constants";
import {
  buildTailOutput,
  isAsyncFiniteCommand,
  isBackgroundCommand,
  looksLikeReadyOutput,
...
export async function runProjectCommandTool(
  ...
  return await new Promise<ToolResult>((resolve) => {
    ...
    const appendChunk = async (chunk: string): Promise<void> => {
      ...
      if (
        backgroundMode &&
        !releasedToBackground &&
        looksLikeReadyOutput(capturedOutput)
      ) {
    ...
    child.on("close", async (code) => {
      ...
      const result = Object.freeze({
        ...
        ...(success
          ...
          : {
              error: timedOut
                ? `Project command timed out: ${commandLabel}`
                : `Project command failed: ${commandLabel}`,
            }),
        meta: Object.freeze({
          commandId: resolved.commandId,
          commandLabel,
