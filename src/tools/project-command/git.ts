...
import { buildShellEnvironment, resolveCommandBinary } from '../../runtime/shell-resolver';
...
export function gitStatusTool(workspacePath: string, options?: Readonly<{ cwd?: string; short?: boolean; pathspec?: string }>): Promise<ToolResult> {
  const args = options?.short ? ['status', '--short'] : ['status'];
  if (options?.pathspec?.trim()) {
    args.push('--', options.pathspec.trim());
  }
  return runDirectBinaryTool({ workspacePath, cwd: options?.cwd, binary: 'git', args, label: 'git status', category: 'git' });
}

/** Runs `git diff` or `git diff --staged` for the requested workspace path. */
export function gitDiffTool(workspacePath: string, options?: Readonly<{ cwd?: string; pathspec?: string; staged?: boolean; maxChars?: number }>): Promise<ToolResult> {
  const args = ['diff'];
  if (options?.staged) {
    args.push('--staged');
  }
...
export function gitAddTool(workspacePath: string, paths: readonly string[], options?: Readonly<{ cwd?: string }>): Promise<ToolResult> {
...
export function gitCommitTool(workspacePath: string, message: string, options?: Readonly<{ cwd?: string; all?: boolean }>): Promise<ToolResult> {
  const args = ['commit'];
  if (options?.all) {
    args.push('-a');
  }
  args.push('-m', message);
  return runDirectBinaryTool({ workspacePath, cwd: options?.cwd, binary: 'git', args, label: 'git commit', category: 'git' });
}

/** Runs `git push` with optional remote and branch arguments. */
export function gitPushTool(workspacePath: string, options?: Readonly<{ cwd?: string; remote?: string; branch?: string }>): Promise<ToolResult> {
  const args = ['push'];
  ...
  return runDirectBinaryTool({ workspacePath, cwd: options?.cwd, binary: 'git', args, label: 'git push', category: 'git' });
}

/** Runs `git pull` with optional remote and branch arguments. */
export function gitPullTool(workspacePath: string, options?: Readonly<{ cwd?: string; remote?: string; branch?: string }>): Promise<ToolResult> {
  const args = ['pull'];
  if (options?.remote?.trim()) {
    args.push(options.remote.trim());
  }
  ...
  return runDirectBinaryTool({ workspacePath, cwd: options?.cwd, binary: 'git', args, label: 'git pull', category: 'git' });
}

/** Runs `git checkout` or `git checkout -b` for the requested ref. */
export function gitCheckoutTool(workspacePath: string, ref: string, options?: Readonly<{ cwd?: string; createBranch?: boolean }>): Promise<ToolResult> {
  const args = ['checkout'];
  if (options?.createBranch) {
    args.push('-b');
  }
  args.push(ref);
  return runDirectBinaryTool({ workspacePath, cwd: options?.cwd, binary: 'git', args, label: 'git checkout', category: 'git' });
}
