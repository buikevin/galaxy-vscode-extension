 ...
 */
export function getAvailableExtensionTools(
  config: GalaxyConfig,
): readonly DiscoveredExtensionTool[] {
...
function getToolCapability(
  toolName: string,
): keyof ToolCapabilityConfig | null {
...
export function isToolEnabled(toolName: string, config: GalaxyConfig): boolean {
  ...
  const capabilityEnabled = capability
    ? config.toolCapabilities[capability]
    : true;
  const toolEnabled =
    normalized in config.toolToggles
